...
Proxmox [9 just dropped](https://www.proxmox.com/en/about/company-details/press-releases/proxmox-virtual-environment-9-0). It now has support for ZFS 2.3.3. ZFS has had raidz expansion available since ZFS 2.3
 ...
 **#2** – You have a pool – what is everything named? After the pool is created, take a look at it by clicking details in the proxmox ui.  ![alt text](image-4.png)
 
 **#3**
Query the pool from cli and see the result  
...
**#4 Install and extend the zpool to the new drive.** 
 ...
 (this can only be done from CLI)    syntax for the command is zpool attach <poolname> <raidlevel> <device or disk>    
 
 my example:   `zpool attach deadpoul raidz1-0 /dev/disk/by-id/ata-KINGSTON_SA400S37120G_50026B76837C5C26`   ![alt text](image-6.png) 
...
![a picture of a command line with the result of zpool status showing 4 disk drives](image-7.png):  |
